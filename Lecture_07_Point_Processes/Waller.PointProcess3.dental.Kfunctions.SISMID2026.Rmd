---
title: "SISMID 2026 Spatial Statistics Waller Point Process 3: K functions Archaeology Dental Example"
...
This code reproduces the K function figures for early medieval gravesites.
...
First, set the library (splancs), set the working directory (set to your own), reading in the data, and read in the polygon boundary file.   
...
here::i_am("Waller.PointProcess3.dental.Kfunctions.SISMID2026.Rmd")
...
# dental$y has a bigger range so this code adds half of the
# extra to each side
...
Next, we'll set things up to compare the gravesite locations to complete spatial randomness (CSR) within a bounding rectangle...not really the question we want to answer, since the graves occur only in the irregular polygon, but a point of comparison for proper analysis.
...
These look like clustering at almost all distances, but these are checking for clustering within a rectangle around the entire area, but the gravesites only occur in the "7" shaped  polygon.
...
In addition to simulating CSR (within the polygon) as the null hypothesis, we can also consider a "random labeling" null hypothesis where we keep set of locations fixed and simply select 30 of them to be cases each time (that is, we randomize the affected/non-affected labels rather than randomizing locations).   The two null hypotheses are close but not identical. (see discussion in Waller and Gotway 2004).
...
## Differences between K functions for cases versus controls 
...
In this case, we'll use the **random labelling** null hypothesis, so each simulation
keeps the same set of all locations and assigns 30 of them to be cases and the others 
to be controls.  This answers a specific question:  "How unusual is our observed pattern
of clustering, when compared to uniformly selecting 30 cases from our full set of 
locations?"  

If we used a CSR null hypothesis for cases and controls, how would the question we answer differ from the random labelling question?   

Which question more accurately reflects our question of interest for comparing clustering
of cases to that of controls?
...
title("Difference in K functions")
```

***

Let's think about the distances compared to the dimensions of our study area.

We likely are really only interested in clustering that occurs for the shorter
distances, so let's focus on distances between 0 and 2000.
...
This plot shows the difference in K functions (thick black line), the null value
(constant at zero, dotted line), and two sets of "tolerance envelopes".  The thin, solid
lines represnt the minimum and maximum difference in K functions observed at each distance
and the two dashed lines represent the 2.5th and 97.5th percentiles of the differences
observed at that distance for each of the random labelling simulations, i.e., these show
what we would expect the central 95% of differences between K functions if 30 of the 
observed locations were chosen as cases where each location has the same chance of
being a case.

For almost every distance, the observed difference in K functions falls within the 
95% tolerance bands, but the difference does approach the lower bound ($K_{case}$ is lower than $K_{control}$) at a distance
of about 350, and approaches the higher bound ($K_{case}$ is higher than $K_{control}$) at distances between that and 500.   What would
...
- K functions summarize how events cluster (at what distance) but do not show
  where clusters occur.
  
- K functions average this tendency over all points.

- CSR and random labelling represent similar but different null hypotheses, but 
simulation allows us to find out whether the observed K function differs from what would be expected under these hypotheses.