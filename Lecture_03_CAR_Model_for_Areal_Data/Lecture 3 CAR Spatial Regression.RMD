...
# SISMID Spatial Statistics
# Lecture 3 Disease Mapping with Conditional Autoregressive Models
***

# 3.1 Introduction to Conditional Autoregressive Models
...
here::i_am("Lecture 3 CAR Spatial Regression.Rmd")
load (here("Data_Areal.RData") )
st_drop_geometry(dat.areal)[1:4,]
```

CAR models for **Normal outcome** can be fitted using the *spautolm* function in library *spatialreg*. The function takes a regression formula similar to *lm*, with a spatial weight matrix defined as an *nb2listw* object.  We will examine both first-order and second-order (neighbors of 1-first order neighbors).
...
The maximum likelihood estimate (MLE) for the incidence rate is given by $Y_i/P_i$. We also calculate the standard error associated with $log (Y_i/P_i)$, which we will compare to estimates from spatial models.
